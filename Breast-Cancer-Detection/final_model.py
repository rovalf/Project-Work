...
# >>> Add this small map and derive IMG_SIZE from BACKBONE <<<
...
def parse_patient_id(fname: str) -> str:
    ...
    while parts and parts[0].lower() in ("benign","malignant","mass","calc"):
        parts = parts[1:]
    for p in parts:
        if re.match(r"[A-Za-z]*\d+", p): return p
...
for fold,(tr,va) in enumerate(folds,1):
    ...
    # After: model,base = build_model(),None
    model = build_model()
    # Find the EfficientNet backbone (first nested keras.Model)
    ...
    cbs=[keras.callbacks.ModelCheckpoint(ckpt,monitor="val_prc",mode="max",save_best_only=True,verbose=1),
         keras.callbacks.EarlyStopping(monitor="val_prc",mode="max",patience=4,restore_best_weights=True,verbose=1)]
    model.fit(train_ds,validation_data=val_ds,epochs=EPOCHS_WARMUP,callbacks=cbs,verbose=2)

    base.trainable=True
    for l in base.layers[:-UNFREEZE_LAST_N_LAYERS]: l.trainable=False
    compile_for("finetune",model,LR_FINETUNE)
    model.fit(train_ds,validation_data=val_ds,epochs=EPOCHS_FINETUNE,callbacks=cbs,verbose=2)

    # end of each fold, after training
    ...
    model.save(native_ckpt)                         # full Keras v3 model (no Lambda issues)
    model.save_weights(weights_ckpt)                # pure weights fallback (HDF5)
